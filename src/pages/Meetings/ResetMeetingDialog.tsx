...
export const ResetMeetingDialog: React.FC<EditMeetingDialogProps> = ({
  open,
  onClose,
  onUpdated,
  meeting,
  ...
  // Reset form và load data khi mở dialog
  useEffect(() => {
    if (open && meeting) {
      loadMeetingData();
      fetchUsers();
  ...
  // Load meeting data vào form
  const loadMeetingData = useCallback(async () => {
    if (!meeting) return;

    try {
      // Fetch chi tiết meeting để có thông tin participants đầy đủ
      const meetingDetail = await meetingService.getMeeting(meeting.id);
      const fullMeetingData = meetingDetail.data;

      // Extract participants info
      const chuTri = fullMeetingData.participants.find((p: any) => p.role === "CT");
      const thanhVien = fullMeetingData.participants
        .filter((p: any) => p.role === "TV")
        .map((p: any) => p.userId);

      const newFormData: FormData = {
        name: fullMeetingData.name || "",
        description: fullMeetingData.description || "",
        startTime: fullMeetingData.startTime ? new Date(fullMeetingData.startTime).toISOString().slice(0, 16) : "",
        endTime: fullMeetingData.endTime ? new Date(fullMeetingData.endTime).toISOString().slice(0, 16) : "",
        roomId: fullMeetingData.roomId || "",
        chuTri: chuTri ? chuTri.userId : null,
        thanhVien: thanhVien,
      };
      ...
      if (newFormData.startTime && newFormData.endTime) {
        fetchRooms(newFormData.startTime, newFormData.endTime, fullMeetingData.roomId);
      }

    } catch (err) {
      console.error("Lỗi khi tải thông tin cuộc họp:", err);
      setSubmitError("Không thể tải thông tin cuộc họp");
    }
  }, [meeting]);
  ...
  const fetchRooms = useCallback(async (start: string, end: string, currentRoomId?: number) => {
    if (!start || !end) return;
    ...
    try {
      ...
      if (formData.roomId && !availableRooms.some((room: Room) => room.roomId === formData.roomId)) {
  ...
  const hasChanges = (): boolean => {
    ...
    return (
      ...
      formData.roomId !== originalData.roomId ||
      formData.chuTri !== originalData.chuTri ||
      JSON.stringify(formData.thanhVien) !== JSON.stringify(originalData.thanhVien)
    );
  };
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      alert("Đã tạo cuộc họp thành công!");
      onUpdated();
      onClose();
      navigate("/meetings");
    } catch (err: any) {
      ...
      if (err.response?.data?.message) {
        ...
        setSubmitError("Có lỗi xảy ra khi cập nhật cuộc họp");
      }
    } finally {
      setLoading(false);
    }
    ...
    >
      ...
      <DialogActions sx={{ p: 3, gap: 1 }}>
        ...
        <Button 
          ...
          color="warning"
        >
          {loading ? "Đang cập nhật..." : "Tạo cuộc họp"}
        </Button>
      </DialogActions>
    </Dialog>
  );
